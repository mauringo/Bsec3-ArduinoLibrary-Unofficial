 ...
 */
void checkBsecStatus(Bsec3 bsec);

/**
 * @brief : This function is called by the BSEC library when a new output is available
 * @param[in] input     : BME69X sensor data before processing
 * @param[in] outputs   : Processed BSEC BSEC output data
 * @param[in] bsec      : Instance of BSEC2 calling the callback
 */
void newDataCallback(const bme69xData data, const bsecOutputs outputs, Bsec3 bsec);
...
void setup(void)
{
    ...
    if (!envSensor.begin(BME69X_I2C_ADDR_LOW, Wire))
    {
...
void checkBsecStatus(Bsec3 bsec)
{
    ...
    if (bsec.sensor.status < BME69X_OK)
    {
        Serial.println("BME69X error code : " + String(bsec.sensor.status));
        errLeds(); /* Halt in case of failure */
    }
    else if (bsec.sensor.status > BME69X_OK)
    {
        Serial.println("BME69X warning code : " + String(bsec.sensor.status));
    }
}